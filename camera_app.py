...
*{box-sizing:border-box}body{margin:0;background:#f7faf9;color:var(--ink);font:14px Segoe UI,Arial,sans-serif}
...
<main class="wrap"><div class="intro"><div><h2>Minha câmera ao vivo</h2><p>Sem pós-processamento pesado: apenas ajustes leves e instantâneos.</p></div><div class="actions"><button id="start" class="btn" onclick="startCamera()">Ligar câmera</button><button id="stop" class="btn gray" onclick="stopCamera()">Parar</button><button id="record" class="btn gray" onclick="toggleRecording()">Gravar câmera</button><button id="virtual" class="btn gray" onclick="toggleVirtual()">Câmera virtual OBS</button></div></div>
<div class="layout"><section class="card"><div class="live-label">CÂMERA AO VIVO</div><div class="preview"><video id="video" autoplay muted playsinline></video><div id="empty" class="empty">Clique em “Ligar câmera” para começar.</div><canvas id="recordCanvas" width=1280 height=720 style="display:none"></canvas></div></section>
